...
def draw_boxes_anana(detections, image, colors, network):
    import cv2
    ...
    for label, confidence, bbox in detections:
        ...
        scale = img_width/net_width

        left = int(left * img_width/net_width - scale*3)
        right = int(right * img_width/net_width + scale*3)
        top = int(top * img_height/net_height - scale*3)
        bottom = int(bottom * img_height/net_height + scale*3)
        ...
        text_color = (10, 0, 0) #azul escuro do tema bootstrap BGR
        rectangle_bgr = colors[label]
        (text_width, text_height) = cv2.getTextSize(text, font, fontScale=fontScale, thickness=thickness)[0]
        text_offset_x = left-int(scale)
        text_offset_y = top-int(scale)
        box_coords = ((text_offset_x, text_offset_y), (text_offset_x + text_width + 2, text_offset_y - text_height - 2))

        #cv2.rectangle(img, box_coords[0], box_coords[1], rectangle_bgr, cv2.FILLED)
        #cv2.putText(img, text, (text_offset_x, text_offset_y), font, fontScale=font_scale, color=(0, 0, 0), thickness=1)

        cv2.rectangle(image, (left, top), (right, bottom), colors[label], int(scale*1.5))
        cv2.rectangle(image, box_coords[0], box_coords[1], rectangle_bgr, cv2.FILLED)
        cv2.putText(image, text, (text_offset_x, text_offset_y), cv2.FONT_HERSHEY_PLAIN, fontScale, text_color, thickness)
    return image
